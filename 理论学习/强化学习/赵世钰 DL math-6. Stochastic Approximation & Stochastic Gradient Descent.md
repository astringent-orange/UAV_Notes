...
回顾之前的内容 mean estimation，通过统计平均值去近似X的期望；MC中是通过统计平均去求当前状态价值的期望
...
这里x就是X的带噪观测值，只是要修正为期望为0的形式；回到mean estimation，求EX，是用X的观测值x
![[63da20a9-b399-4997-8510-131aa2e53afc.png]]

# Stochastic gradient descent
SGD假设有以下这样的优化问题，其中w为可修改的参数，而X是一个分布确定的随机变量。而已知最小值处梯度为0，为了求这样的优化问题，逐步有以下三个方法。
在此之前，有一些数学上的内容需要注意。f(w,X)中有一个随机变量X，所以f本身也是一个随机变量，可以求期望，又因为X的分布是确定的（但是未知），所以求期望以后可以消去X，变为仅是w的函数。同理，f求导数后仍然是随机变量
...
从而有当wk离w* 很远时，上式的分母较大，相对误差是较小的

2. deterministic formulation
前面f中有一个随机变量X，可以进行随机采样，但如果只有确定的数该怎么办？
![[7ca4c314-6385-49ac-9482-7dd954e15874.png]]
其中xi是n个确定的数。可以强行引入随机变量，![[5b19a6bc-ee47-40a0-bbe1-3f20e7075036.png]]


# BGD MBGD SGD
![[091a0357-ae30-4457-b275-e1afffa1442e.png]]
BGD和SGD可以视为MBGD的极端情况；但是还是有细微差别，即BGD是使用所有n个采样，而MBGD是在n个采样中随机抽取了n次，是可能有重复的